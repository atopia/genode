...
class Core::Vcpu : public Rpc_object<Vm_session::Native_vcpu, Vcpu>
{
	...
	public:
		...
		:
			_id(id),
			_ep(ep),
			_constrained_md_ram_alloc(constrained_md_ram_alloc),
			_ds_cap( {_constrained_md_ram_alloc.alloc(vcpu_state_size(), Cache::UNCACHED)} ),
		...
		void exception_handler(Signal_context_capability handler)
		{
			...
			if (!_kobj.create(cpu, (void *)&_vcpu_data, Capability_space::capid(handler), _id))
