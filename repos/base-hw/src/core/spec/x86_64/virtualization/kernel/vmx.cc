...
void Vmcs::switch_world(Core::Cpu::Context &regs)
{
	...
	asm volatile(
	    ...
	    : [regs]           "r"(&regs.r8),
	      ...
	      [trap_vmx_error] "i"(TRAP_VMX_ERROR)
	    : "memory");
}
...
uint64_t Vmcs:: handle_vm_exit(addr_t trapno)

{
	if (trapno == TRAP_VMX_ERROR) {
		/* For error codes, see
		 * Intel SDM (September 2023) Vol. 3C
		 * 31.4 Vm Instruction Error Numbers
		 */
		Kernel::panic(
		    "VM: execution error: ",
		    Genode::Hex(read(Vmcs::E_VM_INSTRUCTION_ERROR)));
	}
