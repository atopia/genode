...
class Core::Phys_allocated : Genode::Noncopyable
{
	...
	public:
		...
		Phys_allocated(Rpc_entrypoint &ep, Ram_allocator &ram, Region_map &rm)
		:
		...
		Phys_allocated(Rpc_entrypoint &ep, Ram_allocator &ram, Region_map &rm, auto const &construct_fn)
		:
		_ep(ep), _ram(ram), _rm(rm)
		{
